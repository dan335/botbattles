...
module.exports = function(app) {
  ...
  app.post('/api/addMod', (req, res) => {
    User.findOne({_id:req.session.userId}).select({isAdmin:1}).exec((error, user) => {
      if (user && user.isAdmin) {
        User.updateOne({_id:req.body.userId}, {$set:{isMod:true}}).exec((error, result) => {
          if (error) {
            res.status(500).end();
          } else {
            res.send('success');
  ...
  app.post('/api/mods', (req, res) => {
    User.findOne({_id:req.body.userId}).select({isAdmin:1}).exec((error, user) => {
      if (user && user.isAdmin) {

        User.find({isMod:true}).exec((error, users) => {
          if (error) {
            res.status(500).end();
          } else {
            res.json(users).end();

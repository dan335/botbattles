...
const messageFunctions =  {
  ...
  blasterBulletInitial: function(json, manager) {
    manager.projectiles.push(new BlasterBullet(manager, Number(json.x), Number(json.y), Number(json.rotation), json.id));
  },

  blasterBulletUpdate: function(json, manager) {
    const projectile = manager.projectiles.find((o) => {
      return o.id == json.id;
    })

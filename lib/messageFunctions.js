...
const messageFunctions =  {
  ...
  spectatorInitial: function(json, manager, ws, ui) {
    var elm = document.getElementById('cooldownContainer');
    if (elm) {
      elm.style.display = 'none';
    }

    elm = document.getElementById('barsContainer');
    if (elm) {
      elm.style.display = 'none';
    }
  },
  ...
  pong: function(json, manager, ws, ui) {
    ...
    if (elm) {
      elm.innerHTML = Math.round(manager.ping/2);
    }
  },

  chat: function(json, manager, ws, ui) {
    ui.setState({chats: [...ui.state.chats, {
      msg: json.msg,
      name: json.name,
  ...
  abilityCooldownFull: function(json, manager, ws, ui) {
    ui.cooldownData[Number(json.num)] = {
      lastFired: Date.now() - Number(json.cooldown),
      cooldown: Number(json.cooldown)
    };
  },
  ...
  shipDestroy: function(json, manager) {
    ...
    if (ship) {
      setTimeout(() => {
        ship.destroy(!json.killer || !json.killer.length ? null : json.killer);
      }, manager.renderDelay());
    }
  },
  ...
  grenadeInitial: function(json, manager) {
    manager.abilityObjects.push(new Grenade(manager, Number(json.x), Number(json.y), Number(json.rotation), Number(json.radius), json.id, parseInt(json.color)));
  },
  ...
  mineInitial: function(json, manager) {
    manager.abilityObjects.push(new Mine(manager, Number(json.x), Number(json.y), Number(json.rotation), Number(json.radius), json.id, parseInt(json.color)));
  },
  ...
  vacuumStart: function(json, manager) {
    ...
    if (ship) {
      new VacuumEffect(manager, Number(json.radius), json.id, ship);
    }
  },
  ...
  vacuumEnd: function(json, manager) {
    ...
    if (ship) {
      const obj = ship.abilityObjects.find((o) => {
        return o.id == json.id;
      })
  ...
  teleportTelegraph: function(json, manager) {
    manager.abilityObjects.push(new TeleportEffect(manager, Number(json.x), Number(json.y), Number(json.radius), Number(json.timeout)));
  },

  maxShieldChange: function(json, manager) {
    const ship = manager.ships.find((s) => {
      return s.id == json.shipId;
    });

    if (ship) {
      ship.updateMaxShield(Number(json.newMax));
    }
  }
}

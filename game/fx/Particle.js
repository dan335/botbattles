...
export default class Particle {
  ...
  constructor(manager, options) {
    ...
    this.material = new SpriteMaterial( { color:options.color || 0xaaaaaa, transparent:true, opacity:this.opacity } );
    this.sprite = new Sprite( this.material );
    this.sprite.scale.set(options.scale, options.scale, options.scale);
    this.sprite.position.set(options.x, 0, options.y);
  ...
  tick() {
    ...
    this.sprite.position.set(this.position.x, 0, this.position.y);
